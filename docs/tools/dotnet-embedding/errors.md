...
指定的`-o=DIR`目录名称不存在并且无法创建。 它可能是无效的文件系统名称。

<a name="EM0002" />

### <a name="em0002-option-x-is-not-supported"></a>EM0002：不`X`支持选项
...
该工具不支持该平台`X`。 可能是该工具的另一个版本支持它，或者`X`平台不适用于此环境中。
...
该工具无法使用`xcode-select -p`命令找到当前选定的 Xcode 位置。 请验证是否可以成功运行此命令，并返回正确的 Xcode 位置。

<a name="EM0007" />

### <a name="em0007-could-not-get-the-sdk-version-for-sdk"></a>EM0007：无法获取 "{sdk}" 的 sdk 版本。

此工具无法使用`xcrun --show-sdk-version --sdk {sdk}`命令获取 SDK 版本。 请验证是否可以成功运行此命令，并返回 SDK 版本。
...
错误消息中的体系结构对于目标平台无效。 请验证是否为该`--abi`选项传递了有效的体系结构。
...
工具无法分析为命令行选项`A`提供的语法。 请查看文档中是否有正确的语法。
...
### <a name="em1011-type-t-is-not-generated-because-it-lacks-marshaling-code-with-a-native-counterpart"></a>EM1011：不`T`生成类型，因为它缺少包含本机对应项的封送处理代码。
...
### <a name="em1021-constructor-c-has-default-values-for-which-no-wrapper-is-generated"></a>EM1021：构造`C`函数具有默认值，而不会生成任何包装。

这是构造**warning**函数`C`的默认参数未生成任何额外代码的警告。 最常见的原因是现有方法已具有相同的签名。 例如，在 .NET 中，可以：
...
在这种情况下， `Increment`只会创建两个生成的选择器，这两个均调入 Mono，但对于后者，则不存在包装。

<a name="EM1033" />

### <a name="em1033-method-m-is-not-generated-because-another-method-exposes-the-operator-with-a-friendly-name"></a>EM1033：不`M`生成方法，因为另一个方法使用友好名称公开运算符。

这是一**条警告**，指出`M`不会生成方法，因为另一个方法使用友好名称公开运算符。 (https://msdn.microsoft.com/library/ms229032(v=vs.110).aspx)

<a name="EM1034" />

### <a name="em1034-extension-method-m-is-not-generated-inside-a-category-because-they-cannot-be-created-on-primitive-type-t-a-normal-static-method-was-generated"></a>EM1034：不会`M`在类别内生成 Extension 方法，因为不能对基元类型`T`创建扩展方法。 已生成正常的静态方法。

这是对**warning** primivite 类型（例如`System.Int32`）的扩展方法的警告。 在客观-C 中，不能对基元类型创建类别。 相反，生成器将生成常规静态方法。
...
### <a name="em1041-indexed-properties-on-t-is-not-generated-because-multiple-indexed-properties-are-not-supported"></a>EM1041：上`T`的索引属性不是生成的，因为不支持多个索引属性。
...
这是一**条警告**，指出`E`元素会生成为`F` ，因为其名称与重要的目标-c 选择器冲突。
...
### <a name="em1052-element-e-is-not-generated-its-name-conflicts-with-other-elements-on-the-same-class"></a>EM1052：未`E`生成元素，其名称与同一类上的其他元素冲突。

这是一个**警告**，指出`E`元素的名称与同一类上的其他元素冲突。

<a name="EM1053" />

### <a name="em1053-target-e-is-not-supported-for-xamarinios-and-xamarinmac-only-the-framework-option-is-considered-supported-and-should-be-used"></a>EM1053：不`E`支持将 Target 用于 Xamarin 和 xamarin。 仅将`framework`选项视为受支持，应使用。

这是针对**warning** Xamarin 和 xamarin `E`使用事例的目标被视为不受支持的警告。 
