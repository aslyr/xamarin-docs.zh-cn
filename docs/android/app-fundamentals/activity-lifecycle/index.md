...
1. *已停止/Backgrounded* &ndash;完全被其他活动遮盖的活动被视为已停止或处于后台。
    已停止的活动仍会尝试尽可能地保留其状态和成员信息，但已停止的活动被视为这三种状态的最低优先级，因此，OS 将先终止此状态的活动，以满足较高优先级活动的资源要求。
...
为了使问题更复杂，Android 在混合环境中多次引发了一次扳手，称为配置更改。 配置更改是指在活动的配置发生更改时（例如，当设备[旋转](~/android/app-fundamentals/handling-rotation.md)时、活动需要在横向或纵向模式下重新生成）、显示键盘（并且活动显示有机会调整其自身大小）或将设备放在停靠中时的快速活动销毁/重新创建循环。
...
[OnStart](xref:Android.App.Activity.OnStart)完成后，系统始终会调用 OnStart `OnCreate` 。 如果活动在活动可见之前需要执行任何特定的任务，例如刷新活动中视图的当前值，则活动可能会重写此方法。 Android 将 `OnResume` 立即调用此方法之后的。
...
`OnResume`很重要，因为在中执行的任何操作都 `OnPause` 应该在中取消 `OnResume` ，因为这是在 `OnPause` 将活动恢复到生活中后保证执行的唯一生命周期方法。
...
- 同样，如果活动显示了任何对话框或警报，则必须用方法对其进行清理 `.Dismiss()` 。
...
`OnStop`不一定会在内存不足的情况下调用，如 Android 对资源会耗尽，无法正确地为活动提供背景。 出于此原因，最好不要依赖于在 `OnStop` 准备用于销毁的活动时调用。 `OnDestroy`如果活动即将消失，或者 `OnRestart` 活动返回到与用户交互，则可以在此之后调用的下一个生命周期方法将是。

#### <a name="ondestroy"></a>OnDestroy

[OnDestroy](xref:Android.App.Activity.OnDestroy)是在将其销毁并从内存中完全删除之前，在活动实例上调用的最终方法。 在极端情况下，Android 可能会终止宿主活动的应用程序进程，这将导致 `OnDestroy` 不会被调用。 大多数活动将不会实现此方法，因为和方法中的大多数清理和关闭操作都已完成 `OnPause` `OnStop` 。 `OnDestroy`通常会重写方法，以清理可能会泄漏资源的长时间运行的资源。 这种情况的一个示例可能是在中启动的后台线程 `OnCreate` 。

在活动被销毁后，不会调用任何生命周期方法。

#### <a name="onrestart"></a>OnRestart

当活动停止后，在重新启动活动之前，将调用[OnRestart](xref:Android.App.Activity.OnRestart) 。 例如，当用户在应用程序中的活动上按下 "主页" 按钮时，就会出现这种情况。 发生这种情况时 `OnPause` `OnStop` ，将调用方法，并将活动移动到后台但不会销毁。 如果用户随后使用任务管理器或类似的应用程序来还原应用程序，则 Android 将调用活动的 `OnRestart` 方法。

对于应在中实现的逻辑类型，没有一般的指导原则 `OnRestart` 。 这是因为 `OnStart` 无论活动是正在创建还是正在重新启动，都将始终调用，因此应在中 `OnStart` （而不是）初始化活动所需的任何资源 `OnRestart` 。

之后调用的下一个生命周期方法 `OnRestart` 将是 `OnStart` 。
...
请记住，当创建了向其 `OnCreate` 传递捆绑作为参数的方法的活动时，可以使用此绑定来还原实例状态。 对于更复杂的数据，不建议使用捆绑包，这些数据不会快速或轻松地序列化为键/值对（如位图）;相反，应将其用于简单的值（如字符串）。
...
- [OnRestoreInstanceState](xref:Android.App.Activity.OnRestoreInstanceState*) &ndash;此方法在方法完成后被调用 `OnCreate` ，并为活动提供了在初始化完成后恢复其状态的另一种机会。
...
当活动正在停止时，将调用[OnSaveInstanceState](xref:Android.App.Activity.OnSaveInstanceState*) 。 它将接收一个捆绑参数，活动可在其中存储其状态。 当设备遇到配置更改时，活动可以使用 `Bundle` 传入的对象通过重写来保留活动状态 `OnSaveInstanceState` 。 例如，考虑以下代码：
...
当单击名为的按钮时，上面的代码会递增名为的整数 `c` `incrementCounter` ，并将结果显示在一个名为的中 `TextView` `output` 。 发生配置更改时（例如，当设备旋转时），上面的代码将丢失的值 `c` ，这是因为应为 `bundle` `null` ，如下图所示：

[![显示不显示上一个值](images/07-sml.png)](images/07.png#lightbox)

若要 `c` 在此示例中保留的值，活动可以重写 `OnSaveInstanceState` ，并将值保存在绑定中，如下所示：
...
> [!NOTE]
> 务必始终调用的基实现， `OnSaveInstanceState` 以便还可以保存视图层次结构的状态。

##### <a name="view-state"></a>视图状态

重写 `OnSaveInstanceState` 是一种适当的机制，用于跨方向更改（如上面示例中的计数器）保存活动中的暂时性数据。 但是， `OnSaveInstanceState` 只要每个视图都分配了 ID，的默认实现就会在 UI 中为每个视图保存暂时性数据。 例如，假设应用程序有一个 `EditText` 在 XML 中定义的元素，如下所示：
...
由于 `EditText` 控件具有已 `id` 分配的，因此，当用户输入某些数据并旋转设备时，数据仍将显示，如下所示：

[![在横向模式下保留数据](images/08-sml.png)](images/08.png#lightbox)

#### <a name="onrestoreinstancestate"></a>OnRestoreInstanceState

[OnRestoreInstanceState](xref:Android.App.Activity.OnRestoreInstanceState*)将在之后调用 OnRestoreInstanceState `OnStart` 。 它为活动提供了在之前保存到捆绑包的任何状态的机会 `OnSaveInstanceState` 。 不过，这是提供给的相同捆绑包 `OnCreate` 。

下面的代码演示如何在中还原状态 `OnRestoreInstanceState` ：
...
此方法用于在应还原状态时提供一些灵活性。 有时，更适合等待所有初始化完成后再还原实例状态。 此外，现有活动的子类可能只需要从实例状态还原某些值。 在许多情况下，不需要重写 `OnRestoreInstanceState` ，因为大多数活动都可以使用提供给的捆绑来还原状态 `OnCreate` 。

有关使用保存状态的示例 `Bundle` ，请参阅[演练-保存活动状态](saving-state.md)。

#### <a name="bundle-limitations"></a>绑定限制

尽管 `OnSaveInstanceState` 可以轻松地保存暂时性数据，但有一些限制：

- 在所有情况下均不会调用它。 例如，按**Home**或**Back**退出活动将不会导致 `OnSaveInstanceState` 调用。

- 传入的捆绑包 `OnSaveInstanceState` 不适用于大型对象，例如图像。 对于大型对象，如下面所述，保存[OnRetainNonConfigurationInstance](xref:Android.App.Activity.OnRetainNonConfigurationInstance)中的对象更可取。

- 使用捆绑包保存的数据将被序列化，这可能会导致延迟。

绑定状态对于不使用大量内存的简单数据很有用，而*非配置实例数据*对于更复杂的数据或检索开销很大的数据（如从 web 服务调用或复杂的数据库查询）很有用。 非配置实例数据将根据需要保存在对象中。 下一节将介绍如何 `OnRetainNonConfigurationInstance` 通过配置更改来保留更复杂的数据类型。

### <a name="persisting-complex-data"></a>保留复杂数据

除了保留捆绑中的数据以外，Android 还支持通过重写[OnRetainNonConfigurationInstance](xref:Android.App.Activity.OnRetainNonConfigurationInstance)并返回 `Java.Lang.Object` 包含要保存的数据的实例来保存数据。 使用保存状态有两个主要优点 `OnRetainNonConfigurationInstance` ：

- 从返回的对象 `OnRetainNonConfigurationInstance` 对更大、更复杂的数据类型执行良好的工作，因为内存会保留此对象。
...
发生配置更改时（例如，当设备旋转时），代码将重复该过程。 若要重用最初检索到的结果，而不会导致不必要的冗余网络调用，可以使用 `OnRetainNonconfigurationInstance` 保存结果，如下所示：
...
现在当设备旋转时，会从属性中检索原始结果 `LastNonConfiguartionInstance` 。 在此示例中，结果包含一个 `string[]` 包含推文的。 由于 `OnRetainNonConfigurationInstance` 要求 `Java.Lang.Object` 返回，因此 `string[]` 包装在子类的类中 `Java.Lang.Object` ，如下所示：
...
例如，如果尝试将 `TextView` 用作从返回的对象 `OnRetainNonConfigurationInstance` ，将会泄漏活动，如下面的代码所示：
...
在本部分中，我们了解了如何在中保留简单状态数据 `Bundle` ，以及如何在中保存更复杂的数据类型 `OnRetainNonConfigurationInstance` 。
