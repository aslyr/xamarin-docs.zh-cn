...
以下屏幕截图显示了具有一个 `TextView` 和两个 `Button` 的简单 `DialogFragment`：
...
### <a name="dismissing-a-fragment"></a>消除片段
...
下面的代码片段中提供了此过程的示例，该代码片段使用 `AddPreferencesFromIntent` 来创建 `PreferenceFragment`：
