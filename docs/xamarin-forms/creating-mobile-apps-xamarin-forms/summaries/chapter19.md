...
Xamarin.Forms 定义了三个视图来维护集合并显示其元素：
...
然后，对 `DataTemplate` 对象调用 [`SetValue`](xref:Xamarin.Forms.DataTemplate.SetValue(Xamarin.Forms.BindableProperty,System.Object)) 和 [`SetBinding`](xref:Xamarin.Forms.DataTemplate.SetBinding(Xamarin.Forms.BindableProperty,Xamarin.Forms.BindingBase))，以将值与 `Cell` 属性相关联，或在引用 `ItemsSource` 集合中的项属性的 `Cell` 集合上设置数据绑定。 [TextCellListCode  ](https://github.com/xamarin/xamarin-forms-book-samples/tree/master/Chapter19/TextCellListCode) 示例对此进行了演示。
