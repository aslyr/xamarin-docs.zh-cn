...
C # 标记是一组用于简化的帮助器方法和类，以简化在 c # 中生成声明性 Xamarin 的过程。 `Xamarin.Forms.Markup`命名空间中提供了 c # 标记提供的 Fluent API。
...
下面的示例演示如何在[`Entry`](xref:Xamarin.Forms.Entry) c # 中创建对象：
...
此示例使用[`Entry`](xref:Xamarin.Forms.Entry) `TwoWay`绑定创建一个对象，该对象`RegistrationCode`将数据绑定到 viewmodel 的属性。 它被设置为显示在中的特定行[`Grid`](xref:Xamarin.Forms.Grid)，并跨中的所有列。 `Grid` 此外，还设置了的`Entry`高度以及文本的字体大小及其。 `Margin`
...
C # 标记包含`Bind`一个扩展方法以及一些重载，用于在视图可绑定属性和指定属性之间创建数据绑定。 `Bind`方法知道 Xamarin 中包含的大部分控件的默认可绑定属性。 因此，在使用此方法时，通常不需要指定目标属性。 但是，还可以为其他控件注册默认的可绑定属性：
...
此外， `BindCommand`扩展方法可以通过单个方法调用绑定到控件的`Command`默认`CommandParameter`值和属性：
...
默认情况下， `CommandParameter`绑定到绑定上下文。 还可以为`Command`和`CommandParameter`绑定指定绑定路径和源：
...
在此示例中，绑定上下文是一个`Item`实例，因此不需要为`Id` `CommandParameter`绑定指定源。

如果只`Command`需要绑定到，则可以将传递`null`给`parameterPath` `BindCommand`方法的参数。 或者，使用`Bind`方法。

还可以注册其他控件的`Command`默认`CommandParameter`值和属性：
...
内联转换器代码可以通过`Bind` `convert`和`convertBack`参数传递到方法：
...
此外，转换器代码和实例可以与`FuncConverter`类重新使用：
...
还可以将数据绑定到`Span`用`FormattedText`属性指定的对象：
...
`Command`和`CommandParameter`属性可以是使用`BindClickGesture`、 `BindSwipeGesture`和`BindTapGesture`扩展`View`方法绑定到`GestureElement`和类型的数据：
...
此示例将创建指定类型的笔势识别器，并将其添加到[`Label`](xref:Xamarin.Forms.Label)中。 `Bind*Gesture`扩展方法提供的参数与`BindCommand`扩展方法相同。 但是，默认`Bind*Gesture`情况下不绑定`CommandParameter`，而`BindCommand`是。

若要使用参数初始化笔势识别器，请`ClickGesture`使用`PanGesture`、 `PinchGesture`、 `SwipeGesture`、和`TapGesture`扩展方法：
...
由于笔势识别器是`BindableObject`，因此你可以在初始化`Bind`时`BindCommand`使用和扩展方法。 还可以用`Gesture<TGestureElement, TGestureRecognizer>`扩展方法初始化自定义笔势识别器类型。
...
> [!IMPORTANT]
> 使用[`Grid`](xref:Xamarin.Forms.Grid)枚举定义行和列需要以下`using`指令：`using static Xamarin.Forms.Markup.GridRowsColumns;`
...
以下列表`FontSize`中的控件可以调用、、 `Bold` `Italic`和`Font`扩展方法，以设置控件显示的文本的外观：
...
> [!NOTE]
> `Style`除了扩展方法之外，还有`ApplyToDerivedTypes`、 `BasedOn` `Add`、和`CanCascade`扩展方法。
...
`Invoke`扩展方法可用于应用平台细节。 但是，若要避免出现多义性错误， `using`请不要直接`Xamarin.Forms.PlatformConfiguration.*Specific`包含用于命名空间的指令。 请改为创建命名空间别名，并通过别名使用特定于平台的：
...
- **目的**：其值标识控件用途（例如`Text`， `Placeholder`、`Assign`）的任何属性或 helper 方法。
