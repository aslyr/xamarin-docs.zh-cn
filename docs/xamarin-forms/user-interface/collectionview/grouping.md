...
以下屏幕截图显示了[`CollectionView`](xref:Xamarin.Forms.CollectionView)一个显示分组数据的：
...
然后`IEnumerable<T>` ，可以创建组的集合：

```csharp
public List<AnimalGroup> Animals { get; private set; } = new List<AnimalGroup>();
```

此代码定义一个名为`Animals`的集合，其中集合中的每一`AnimalGroup`项都是一个对象。 每`AnimalGroup`个对象都包含一个名称和`List<Animal>`一个定义组中`Animal`的对象的集合。
...
此代码在`Animals`集合中创建两个组。 第一个`AnimalGroup`命名`Bears`为，并包含一`List<Animal>`组熊详细信息。 第二`AnimalGroup`个命名`Monkeys`为，包含猴子`List<Animal>`详细信息的集合。
...
通过将[`CollectionView`](xref:Xamarin.Forms.CollectionView) [`CollectionView.ItemTemplate`](xref:Xamarin.Forms.ItemsView.ItemTemplate)属性设置为，可以定义中每个项的外观[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)。 有关详细信息，请参阅[定义项外观](~/xamarin-forms/user-interface/collectionview/populate-data.md#define-item-appearance)。
...
可以通过将`CollectionView.GroupHeaderTemplate`属性设置为来自定义每个组标头的[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)外观：
...
在此示例中，每个组标头设置[`Label`](xref:Xamarin.Forms.Label)为，其中显示组名称，并设置了其他外观属性。 以下屏幕截图显示了自定义的组标头：
...
可以通过将`CollectionView.GroupFooterTemplate`属性设置为来自定义每个组脚注的外观[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)：
...
在此示例中，每个组脚注均设置[`Label`](xref:Xamarin.Forms.Label)为，以显示组中的项数。 以下屏幕截图显示了自定义的组脚注：
