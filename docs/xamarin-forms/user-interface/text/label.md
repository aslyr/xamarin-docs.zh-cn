...
可以通过将[`Label`](xref:Xamarin.Forms.Label) `Label.TextDecorations`属性设置为一个或多个`TextDecorations`枚举成员，将下划线和删除线文本修饰应用到实例：
...
以下屏幕截图显示了`TextDecorations`应用于[`Label`](xref:Xamarin.Forms.Label)实例的枚举成员：
...
可以通过将[`Label`](xref:Xamarin.Forms.Label) `Label.CharacterSpacing`属性设置为`double`值，将字符间距应用到实例：
...
结果是，中显示的文本中的字符[`Label`](xref:Xamarin.Forms.Label)是与设备`CharacterSpacing`无关的单元相隔的间距。

## <a name="new-lines"></a>新行

从 XAML 将中的文本强制[`Label`](xref:Xamarin.Forms.Label)插入新行有两种主要方法：
...
可以通过可绑定[`TextColor`](xref:Xamarin.Forms.Label.TextColor)属性将标签设置为使用自定义文本颜色。
...
- **CharacterWrap** &ndash;将文本包装到字符边界处的新行。
...
[`Label`](xref:Xamarin.Forms.Label)可以通过将`Label.MaxLines`属性设置为`int`值来指定显示的行数：

- 如果`MaxLines`为-1 （这是其默认值）， `Label`则会将[`LineBreakMode`](xref:Xamarin.Forms.Label.LineBreakMode)属性的值考虑为仅显示一行（可能已截断）或包含所有文本的所有行。
- 如果`MaxLines`为0，则`Label`不会显示。
- 如果`MaxLines`为1，则结果与将[`LineBreakMode`](xref:Xamarin.Forms.Label.LineBreakMode)属性设置为[`NoWrap`](xref:Xamarin.Forms.LineBreakMode)、 [`HeadTruncation`](xref:Xamarin.Forms.LineBreakMode) [`MiddleTruncation`](xref:Xamarin.Forms.LineBreakMode)、或[`TailTruncation`](xref:Xamarin.Forms.LineBreakMode)相同。 但是，如果`Label`适用，将遵循与省略号[`LineBreakMode`](xref:Xamarin.Forms.Label.LineBreakMode)的位置有关的属性的值。
- 当`MaxLines`大于1时， `Label`将显示多达指定的行数，同时遵从[`LineBreakMode`](xref:Xamarin.Forms.Label.LineBreakMode)属性的值（如果适用）。 但是，如果`MaxLines` [`LineBreakMode`](xref:Xamarin.Forms.Label.LineBreakMode)将属性设置为[`NoWrap`](xref:Xamarin.Forms.LineBreakMode)，则将属性设置为大于1的值将不起作用。

下面的 XAML 示例演示如何在`MaxLines`上设置属性[`Label`](xref:Xamarin.Forms.Label)：
...
[`Label`](xref:Xamarin.Forms.Label)类具有`TextType`属性，该属性确定`Label`实例是否应显示纯文本或 HTML 文本。 应将此属性设置为`TextType`枚举的成员之一：

- `Text`指示`Label`将显示纯文本，并且为`Label.TextType`属性的默认值。
...
在 XAML 中，HTML 字符串可能因另外转义`<`和符号而`>`变得不可读：
...
另外，为了提高可读性，可以在`CDATA`节中内联 HTML：
...
在此示例中， `Label.Text`属性设置为在`CDATA`部分中内联的 HTML 字符串。 这是因为`Text`属性是`ContentProperty` `Label`类的。

以下屏幕截图显示了[`Label`](xref:Xamarin.Forms.Label)一个显示 HTML：

![在 iOS 和 Android 上显示 HTML 的标签的屏幕截图](label-images/label-html.png)

> [!IMPORTANT]
> 在中[`Label`](xref:Xamarin.Forms.Label)显示 html 仅限于底层平台支持的 html 标记。
...
`FormattedText`属性的类型[`FormattedString`](xref:Xamarin.Forms.FormattedString)为，它包含一个或多个[`Span`](xref:Xamarin.Forms.Span)实例，通过[`Spans`](xref:Xamarin.Forms.FormattedString.Spans)属性进行设置。 以下`Span`属性可用于设置视觉外观：
...
- [`ForegroundColor`](xref:Xamarin.Forms.Span.ForegroundColor)–范围中文本的颜色。 此属性已过时，并且已被`TextColor`属性替换。
...
下面的 XAML 示例说明了`FormattedText`包含三个[`Span`](xref:Xamarin.Forms.Span)实例的属性：
...
> [!IMPORTANT]
> 可以[`Text`](xref:Xamarin.Forms.Span.Text)通过数据绑定`Span`设置的属性。 有关详细信息，请参阅[数据绑定](~/xamarin-forms/app-fundamentals/data-binding/index.md)。

请注意， [`Span`](xref:Xamarin.Forms.Span)还可以响应添加到跨距[`GestureRecognizers`](xref:Xamarin.Forms.GestureElement.GestureRecognizers)集合的任何笔势。 例如，在上面[`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer)的代码示例中，已`Span`将添加到第二个。 因此，点击此`Span`方法时， `TapGestureRecognizer`将通过执行[`Command`](xref:Xamarin.Forms.TapGestureRecognizer.Command)属性定义`ICommand`的进行响应。 有关手势识别器的详细信息，请参阅[Xamarin。](~/xamarin-forms/app-fundamentals/gestures/index.md)
...
可以通过[`Label.LineHeight`](xref:Xamarin.Forms.Label.LineHeight)将属性设置为或[`Span.LineHeight`](xref:Xamarin.Forms.Span.LineHeight)将属性设置`double`为值[`Label`](xref:Xamarin.Forms.Label) ，自定义和的[`Span`](xref:Xamarin.Forms.Span)垂直高度。 在 iOS 和 Android 上，这些值为原始行高的乘数，在通用 Windows 平台（UWP）上， `Label.LineHeight`属性值为标签字体大小的乘数。
...
下面的 XAML 示例演示如何在[`LineHeight`](xref:Xamarin.Forms.Label.LineHeight)上设置属性[`Label`](xref:Xamarin.Forms.Label)：
...
下面的 XAML 示例演示如何在[`LineHeight`](xref:Xamarin.Forms.Span.LineHeight)上设置属性[`Span`](xref:Xamarin.Forms.Span)：
...
填充表示元素及其子元素之间的空间，用于将元素与其自身内容分离。 可以通过将[`Label`](xref:Xamarin.Forms.Label) `Label.Padding`属性设置为[`Thickness`](xref:Xamarin.Forms.Thickness)值，将填充应用到实例：
...
> [!IMPORTANT]
> 在 iOS 上，当[`Label`](xref:Xamarin.Forms.Label)创建设置`Padding`属性的时，将应用填充，并且可以在以后更新填充值。 但是，如果`Label`创建不设置`Padding`属性的，则稍后尝试设置该属性将不起作用。
>
> 在 Android 和通用 Windows 平台上，当`Padding`创建或更高版本时`Label` ，可以指定属性值。
...
1. 设置[`Label`](xref:Xamarin.Forms.Label)或`TextColor` [`Span`](xref:Xamarin.Forms.Span)的`TextDecoration`和属性。
1. 将添加[`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer)到[`GestureRecognizers`](xref:Xamarin.Forms.GestureElement.GestureRecognizers) [`Label`](xref:Xamarin.Forms.Label)或[`Span`](xref:Xamarin.Forms.Span)的集合，其[`Command`](xref:Xamarin.Forms.TapGestureRecognizer.Command)属性绑定到`ICommand`，其[`CommandParameter`](xref:Xamarin.Forms.TapGestureRecognizer.CommandParameter)属性包含要打开的 URL。
1. `ICommand`定义将由执行的[`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer)。
...
点击超链接时， [`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer)将通过执行其`ICommand` [`Command`](xref:Xamarin.Forms.TapGestureRecognizer.Command)属性定义的来做出响应。 此外，通过将[`CommandParameter`](xref:Xamarin.Forms.TapGestureRecognizer.CommandParameter)属性指定的 URL `ICommand`作为参数传递给。
...
`HyperlinkSpan`类`Url`定义属性和关联[`BindableProperty`](xref:Xamarin.Forms.BindableProperty)的，并且构造函数设置超链接的外观，以及[`TapGestureRecognizer`](xref:Xamarin.Forms.TapGestureRecognizer)在点击超链接时将做出响应。 `HyperlinkSpan`点击时， `TapGestureRecognizer`将通过执行`Launcher.OpenAsync`方法来做出响应，以便在 web 浏览器中打开由`Url`属性指定的 URL。

可以`HyperlinkSpan`通过将类的实例添加到 XAML 来使用类：
...
前面几节介绍了[`Label`](xref:Xamarin.Forms.Label)每[`Span`](xref:Xamarin.Forms.Span)个实例的设置和属性。 但是，可以将属性集分组为一个样式，该样式始终应用到一个或多个视图。 这可以提高代码的可读性，并使设计更改更易于实现。 有关详细信息，请参阅[样式](~/xamarin-forms/user-interface/text/styles.md)。
