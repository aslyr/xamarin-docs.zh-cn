...
控件由 Xamarin. Maps NuGet 包提供，此包应添加到解决方案中的每个项目中。 [Xamarin.Forms.Maps](https://www.nuget.org/packages/Xamarin.Forms.Maps/) [`Map`](xref:Xamarin.Forms.Maps.Map)

安装[Xamarin. Maps](https://www.nuget.org/packages/Xamarin.Forms.Maps/) NuGet 包后，必须在每个平台项目中对其进行初始化。

在 iOS 上，此方法应**在 AppDelegate.cs**中出现`Xamarin.FormsMaps.Init` ，方法是`Xamarin.Forms.Forms.Init`在方法*之后*调用方法：

```csharp
Xamarin.FormsMaps.Init();
```

`Xamarin.Forms.Forms.Init`在 Android 上，此方法应在**MainActivity.cs**中出现`Xamarin.FormsMaps.Init` ，方法是在方法*之后*调用方法：

```csharp
Xamarin.FormsMaps.Init(this, savedInstanceState);
```

在通用 Windows 平台（UWP）上，这应在**MainPage.xaml.cs**中通过调用`Xamarin.FormsMaps.Init` `MainPage`构造函数中的方法来发生：
...
若要支持 iOS 11 和更早版本，你可以包含所有`NSLocationWhenInUseUsageDescription`三`NSLocationAlwaysAndWhenInUseUsageDescription`个键`NSLocationAlwaysUsageDescription`：、和。
...
1. 可有可无指定清单中的位置权限。
1. 可有可无类中的`MainActivity`请求运行时位置权限。
...
在`<application>` **androidmanifest.xml**的元素中添加以下声明：
...
如果 Xamarin 窗体应用程序面向 API 28 或更高版本，则必须在`<application>` **androidmanifest.xml**的元素中添加以下声明：

```xml
<uses-library android:name="org.apache.http.legacy" android:required="false" />    
```

这会告知应用程序使用 Apache Http 客户端库（已从 Android 9 中的`bootclasspath`中删除）。

#### <a name="specify-the-write_external_storage-permission"></a>指定 WRITE_EXTERNAL_STORAGE 权限

如果应用程序面向 API 22 或更低，则可能需要将`WRITE_EXTERNAL_STORAGE`权限添加到清单，作为`<manifest>`元素的子元素：
...
如果你的应用程序需要访问用户的位置，则必须通过将`ACCESS_COARSE_LOCATION`或`ACCESS_FINE_LOCATION`权限添加到清单（或两者）来请求权限，作为`<manifest>`元素的子元素：
...
1. 在`MainActivity`类中，添加以下`OnStart`重写：
    ...
    如果应用程序面向 API 23 或更高版本，则此代码将为`AccessFineLocation`权限执行运行时权限检查。 如果未授予权限，则通过调用`RequestPermissions`方法发出权限请求。
...
> [!NOTE]
> 在 UWP 上，若要使用地图服务（如地理编码），还`MapService.ServiceToken`必须将属性设置为 "身份验证密钥" 值。 这可以通过以下代码行完成： `Windows.Services.Maps.MapService.ServiceToken = "INSERT_AUTH_TOKEN_HERE";`。
...
UWP 发布版本使用 .NET 本机编译将应用程序直接编译为本机代码。 但是，这种情况的结果是，UWP 上的[`Map`](xref:Xamarin.Forms.Maps.Map)控件的呈现器可能与可执行文件链接。 这可以通过在`Forms.Init` **App.xaml.cs**中使用方法的 UWP 特定重载来修复：
...
此代码将`Xamarin.Forms.Maps.UWP.MapRenderer`类所驻留的程序集传递给`Forms.Init`方法。 这可确保 .NET 本机编译过程不会将程序集链接到可执行文件。
