...
Xamarin。窗体应用程序可以使用`DynamicResource`标记扩展在运行时动态响应样式更改。 此标记扩展类似于`StaticResource`标记扩展，因为这两种方法都使用字典键从中提取值。 [`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary) 但是，当`StaticResource`标记扩展执行单个字典查找时， `DynamicResource`标记扩展将保留指向字典键的链接。 因此，如果替换与该键关联的值，则更改将应用到[`VisualElement`](xref:Xamarin.Forms.VisualElement)。 这使运行时主题可以在 Xamarin. Forms 应用程序中实现。
...
[![Screenshot of the main page of a themed app, on iOS and Android](theming-images/main-page-both-themes.png "主题应用的主页")](theming-images/main-page-both-themes-large.png#lightbox "主题应用的主页")
在[![ios 和 android 上的主题应用的详细信息页](theming-images/detail-page-both-themes.png "主题应用的详细信息页")的 ios 和 android 屏幕截图上，主题应用的主页的屏幕截图](theming-images/detail-page-both-themes-large.png#lightbox "主题应用的详细信息页")
...
> [!IMPORTANT]
> 对于调用`ResourceDictionary` `InitializeComponent`方法的每个，都需要代码隐藏文件。 这是必需的，以便可以在运行时创建表示所选主题的 CLR 对象。

## <a name="set-a-default-theme"></a>设置默认主题

应用程序需要默认主题，以便控件具有其使用的资源的值。 可以通过将主题合并[`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)到在`ResourceDictionary` **应用**程序中定义的应用程序级来设置默认主题：
...
当应用程序想要使用存储在中[`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)的资源表示主题时，它应使用`DynamicResource`标记扩展来实现。 这可确保在运行时选择其他主题时，将应用新主题中的值。
...
当直接使用主题资源时，应使用`DynamicResource`标记扩展来使用该资源。 但是，如果使用`DynamicResource`标记扩展的样式使用，则应使用`StaticResource`标记扩展来使用该样式。
...
1. 从应用程序中删除当前主题。 这是通过清除应用程序[`MergedDictionaries`](xref:Xamarin.Forms.ResourceDictionary.MergedDictionaries)级别[`ResourceDictionary`](xref:Xamarin.Forms.ResourceDictionary)的属性来实现的。
2. 加载选定的主题。 这是通过将所选主题的实例添加到应用程序`MergedDictionaries`级`ResourceDictionary`的属性来实现的。

使用[`VisualElement`](xref:Xamarin.Forms.VisualElement) `DynamicResource`标记扩展设置属性的任何对象将应用新的主题值。 之所以发生这种`DynamicResource`情况，是因为标记扩展会保留一个到字典键的链接。 因此，当替换与键关联的值时，所做的更改将应用`VisualElement`于对象。
