...
其中一项更改是在用户长按某个条形项时显示的新的 HUD 显示。 若要启用此设置，请 `largeContentSizeImage` 在上设置属性 `UIBarItem` ，并通过[资产目录](~/ios/app-fundamentals/images-icons/displaying-an-image.md)添加更大的图像：
...
iOS 11 引入了新功能，使导航栏标题更易于阅读。 应用可通过将属性分配给 true 来显示此更大的标题 `PrefersLargeTitles` ：
...
根据你的应用程序的功能，你可能需要也可能不希望在用户滚动列表时隐藏搜索栏。 您可以使用属性对此进行调整 `HidesSearchBarWhenScrolling` 。
...
在 iOS 10 和之前的版本中，所有视图都具有与之对齐的最小边距大小。 iOS 11 引入了使用替代的选项 `ViewRespectsSystemMinimumLayoutMargins` 。 例如，如果将此属性设置为 false，则可以将边缘嵌入调整为零：
...
iOS 7[引入](~/ios/platform/introduction-to-ios7/ios7-ui.md#fullscreen) `topLayoutGuide` `bottomLayoutGuide` 了并作为一种约束视图的方式，使其不会被 UIKit 条隐藏，并且位于屏幕的可见区域。 它们已在 iOS 11 中弃用，以支持_安全区域_。
...
以前，如果你已添加了一个视图，并希望它在上述绿色区域中可见，则会将其限制为的_底部_ `TopLayoutGuide` 和_顶部_ `BottomLayoutGuide` 。 在 iOS 11 中，你应将其限制在安全区域的_顶部_和_底部_。 以下是一个示例：
...
默认情况下，页眉、页脚和单元格现在会根据其内容自动调整大小。 若要选择退出此自动调整大小行为 `EstimatedRowHeight` ，请将、 `EstimatedSectionHeaderHeight` 或设置 `EstimatedSectionFooterHeight` 为零。
