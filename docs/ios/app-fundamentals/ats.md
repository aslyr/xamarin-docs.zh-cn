...
对于新的 Xamarin iOS 应用，应 `HTTPS` 在与 internet 资源通信时独占使用。 同样，在某些情况下（例如使用第三方 web 服务），这是不可能的，你需要选择退出 ATS。
...
> 应用传输安全已阻止明文 HTTP （ `http://www.-the-blocked-domain.com` ）资源加载，因为它不安全。 临时异常可以通过应用的 info.plist 文件进行配置。
...
Apple 还提供可编译（或者可以选择转码到 Xamarin 和 c #）的[TLSTool 示例应用](https://developer.apple.com/library/mac/samplecode/sc1236/Introduction/Intro.html#//apple_ref/doc/uid/DTS40014927-Intro-DontLinkElementID_2)，并用于诊断 ATS/TLS 问题。 有关如何解决此问题的信息，请参阅下面的 ATS 部分中的 "选择[退出](#optout)" 部分。
...
- **NSAllowsArbitraryLoadsInWebContent** （ `Boolean` ）-如果 `YES` 将允许正确加载网页，而仍为应用的其余部分启用 APPLE 传输安全（ATS）保护。
...
- **NSExceptionMinimumTLSVersion** （ `String` ）-最小 TLS 版本为 `TLSv1.0` `TLSv1.1` 或 `TLSv1.2` （这是默认值）。
- **NSExceptionRequiresForwardSecrecy** （ `Boolean` ）-如果 `NO` 域不必使用带有转发安全的密码。 默认值为 `YES`。
- **NSExceptionAllowsInsecureHTTPLoads** （ `Boolean` ）-如果 `NO` 为（默认值），则与此域的所有通信都必须在 `HTTPS` 协议中。
