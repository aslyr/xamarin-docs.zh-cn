...
如果在 tvOS 应用程序中创建基于的自定义类型 `UIKit` （例如 `UIView` ），还可以通过事件提供低级别的按钮按下处理 `UIPress` 。

`UIPress`事件是 tvOS `UITouch` 事件对 iOS 的作用，但会 `UIPress` 返回有关 Siri 远程或其他连接的蓝牙设备（例如游戏控制器）的按钮按下的信息。 `UIPress`事件描述按下的按钮及其状态（开始、取消、更改或结束）。
...
- 已将微游戏控制器配置文件（ `GCMicroGamepad` ）添加为面向 Siri 远程。
